...
class GeometryOptimizerTests(unittest.TestCase):
    ...
    def test_design_builds_a_true_180_degree_mirror(self) -> None:
        design = GeometryDesign(200, 160, 180, 0, 250, 60, -90, 0, 0)
    ...
    def test_simulator_starts_with_both_arms_folded_at_home(self) -> None:
        ...
        try:
            for arm_id in ArmId:
                ...
                # Folded rest keeps the elbow joint off the board side of the base.
                if arm_id is ArmId.WHITE:
                    self.assertLessEqual(tool.y_mm, cfg.base_y_mm + 30.0)
                else:
                    self.assertGreaterEqual(tool.y_mm, cfg.base_y_mm - 30.0)
